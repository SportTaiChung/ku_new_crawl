...
function core_hmac_md5(n, t) {
    ...
    for (r.length > 16 && (r = core_md5(r, n.length * chrsz)),
    u = Array(16),
    f = Array(16),
    i = 0; i < 16; i++)
        u[i] = r[i] ^ 909522486,
        f[i] = r[i] ^ 1549556828;
    return e = core_md5(u.concat(str2binl(t)), 512 + t.length * chrsz),
...
function str2binl(n) {
    for (var i = Array(), r = (1 << chrsz) - 1, t = 0; t < n.length * chrsz; t += chrsz)
...
function binl2hex(n) {
    for (var i = hexcase ? "0123456789ABCDEF" : "0123456789abcdef", r = "", t = 0; t < n.length * 4; t++)
        r += i.charAt(n[t >> 2] >> t % 4 * 8 + 4 & 15) + i.charAt(n[t >> 2] >> t % 4 * 8 & 15);
    return r
}
function binl2b64(n) {
    for (var u, i, r = "", t = 0; t < n.length * 4; t += 3)
        for (u = (n[t >> 2] >> 8 * (t % 4) & 255) << 16 | (n[t + 1 >> 2] >> 8 * ((t + 1) % 4) & 255) << 8 | n[t + 2 >> 2] >> 8 * ((t + 2) % 4) & 255,
        i = 0; i < 4; i++)
            r += t * 8 + i * 6 > n.length * 32 ? b64pad : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(u >> 6 * (3 - i) & 63);
    return r
}
var hexcase = 0
  , b64pad = ""

...
def on_open(ws):
    ...
    ws.send('{"action": "orderR", "date": "", "ball": 0, "dc": 1}')
    time.sleep(keepLive_time)
    BBKeepLive(ws)

def openWebSocket(url, option, protocol):
...
def goToLoby(lobyUrl):
    ...
    if r.status_code == 200:
        ...
        openWebSocket("wss://" + dataJson["WRWSUrl"][0] + "/", dataJson["BBUrlSearch"], dataJson["BBProtocol"])
